...
#     tmp_images = os.path.join(home, "videos")
...
#             tmp_images, f"{video_counter}_image_{i + 1}.jpg")
...
def clear_files():
    for filename in Path(home).glob(f"tmp_images/*.jpg"):
        os.remove(filename)
        
    for filename in Path(home).glob(f"videos/*.mp4"):
        os.remove(filename)
...
def copy_images(batch_num, row, my_hash):
    with capture_lock:
        ...
        # Create the "tmp_images" folder if it doesn't exist
        folder_path = os.path.join(home, f"tmp_images")
...
def index(path='index.html'):
    fp = os.path.join(home, "dashboard/dist/", path)
    ...
    while my_hash == scan_hash:
        ...
        for pending_result in pending_results_tracker:
            ...
            if result["status"] != "success":
                ...
                path = os.path.join(home, f"inference/{pending_result['batch']}.jpg")
                image.save(path, 'JPEG')
                results_table[pending_result["batch"]]["inference_location"] = os.path.join(home, f"inference/{pending_result['batch']}.jpg")
...
if __name__ == '__main__':
    ...
    video_folder_path = os.path.join(home, "videos")
    if not os.path.exists(video_folder_path):
        os.makedirs(video_folder_path)

    inference_folder_path = os.path.join(home, "inference")
